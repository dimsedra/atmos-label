...
function Mark() {
  return (
    <a href="#top" className="brand-mark" aria-label="ATMOS home">
      ATMOS<span>®</span>
    </a>
  );
}
...
function SectionHead({ index, title, link, dark = false }: { index: string; title: string; link: string; dark?: boolean }) {
  return (
    <div className={`flex items-end justify-between border-b pb-5 ${dark ? 'border-white/30' : 'border-black/30'}`}>
      <div className="flex items-baseline gap-4 md:gap-8">
        <span className={`text-[10px] font-semibold ${dark ? 'text-[#e8ff43]' : 'text-[#ff3b26]'}`}>{index}</span>
        <h2 className="text-[clamp(2.8rem,6vw,6.5rem)] font-semibold leading-none tracking-[-.065em] uppercase">{title}</h2>
      </div>
      <a href="#" className="mb-1 hidden items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] sm:flex">
        {link}
        <ArrowRight size={15} />
      </a>
    </div>
    ...
    <main id="top" className="overflow-hidden bg-[#f4f4f1] text-[#111]">
      <header className="fixed inset-x-0 top-0 z-50 flex h-[74px] items-center justify-between border-b border-white/25 px-5 text-white mix-blend-difference md:px-10">
        ...
        <div className="flex items-center gap-5">
          <a className="hidden text-[11px] font-semibold uppercase tracking-[.18em] sm:block" href="#shop">
            Seoul / KR
          </a>
          <button onClick={() => setMenuOpen(true)} className="p-1 md:hidden" aria-label="Open menu">
            <Menu size={25} />
          </button>
        </div>
      </header>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] flex flex-col bg-[#e9ff45] p-6 text-black"
          >
            ...
            <nav className="mt-auto mb-auto flex flex-col text-[13vw] font-semibold uppercase leading-[.95] tracking-[-.06em]">
              <a onClick={closeMenu} href="#releases">Music</a>
              <a onClick={closeMenu} href="#artists">Artists</a>
              <a onClick={closeMenu} href="#shop">Objects</a>
              <a onClick={closeMenu} href="#journal">Journal</a>
            </nav>
      ...
      <section className="hero relative flex min-h-[100svh] items-end bg-black text-white">
        <motion.img
          ...
          src={hero}
          alt="ATMOS artists in a silver, smoke-filled studio"
        ...
        <div className="relative z-10 w-full px-5 pb-7 md:px-10 md:pb-10">
          <motion.p
            ...
            className="mb-4 max-w-sm text-sm leading-relaxed text-white/80 md:ml-auto md:mb-0"
          >
            A Seoul-born music and lifestyle label shaping sound, image, and objects beyond the expected.
          </motion.p>
          <motion.h1
          ...
          <div className="mt-2 flex items-end justify-between border-t border-white/40 pt-4 text-[10px] font-semibold uppercase tracking-[.22em]">
            ...
            <a href="#releases" className="flex items-center gap-2">
              Enter the atmosphere <ArrowDown size={14} />
            </a>
          </div>
        </div>
      </section>
        ...
        <div className="mt-14 grid items-stretch gap-8 lg:grid-cols-[1.28fr_.72fr]">
          ...
          >
            <img
              ...
              alt="MISO in a silver outfit for the Glasshouse release"
          ...
          <div className="flex flex-col justify-end lg:pl-5">
            ...
            <div className="mt-8 flex items-end justify-between border-b border-black/30 pb-5">
              <div>
                <p className="text-xl font-medium">MISO</p>
            ...
            <a href="#" className="arrow-link mt-5">
              Listen everywhere <ArrowRight size={18} />
            </a>
          </div>
        </div>
      </section>
        ...
        <div className="mt-14 grid grid-cols-2 gap-x-3 gap-y-12 md:grid-cols-4 md:gap-x-5">
          {artists.map((artist, i) => (
            <motion.a
              ...
              className={`group block ${i % 2 ? 'md:mt-24' : ''}`}
            >
              ...
              <div className="mt-4 flex items-start justify-between border-t border-white/25 pt-3">
                <div>
                  <h3 className="text-2xl font-semibold tracking-[-.04em] md:text-3xl">{artist.name}</h3>
                  <p className="mt-1 text-[10px] uppercase tracking-[.16em] text-white/50">{artist.genre}</p>
                </div>
                <ArrowRight className="-rotate-45 transition group-hover:rotate-0" size={18} />
              </div>
            </motion.a>
          ))}
        </div>
      ...
      <section id="journal" className="px-5 py-24 md:px-10 md:py-36">
        ...
        <a href="#" className="group mt-14 grid gap-7 border-t border-black/25 pt-6 md:grid-cols-[1.2fr_.8fr] md:gap-14">
          <div className="aspect-[16/10] overflow-hidden">
            <img
              src="https://images.pexels.com/photos/8221405/pexels-photo-8221405.jpeg?auto=compress&cs=tinysrgb&fit=crop&w=1600&q=90"
              alt="Artists on a rock landscape"
              className="h-full w-full object-cover grayscale transition duration-700 group-hover:scale-[1.02]"
            />
          </div>
          <div className="flex flex-col justify-between">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[.2em] text-black/50">Field Notes 008 · 06.14.26</p>
              <h3 className="mt-6 text-[clamp(2.5rem,5vw,5.6rem)] font-medium leading-[.93] tracking-[-.06em]">
                Outside the system, inside the sound.
              </h3>
              <p className="mt-6 max-w-md text-sm leading-relaxed text-black/60">
                MISO and creative director Han Sol discuss making Glasshouse between Seoul and Jeju, and why independence is a daily practice.
              </p>
            </div>
      ...
      <footer className="bg-[#c9c9c5] px-5 pb-8 pt-20 md:px-10 md:pt-28">
        <div className="grid gap-16 md:grid-cols-2">
          <h2 className="text-[clamp(3.5rem,7vw,7rem)] font-semibold leading-[.88] tracking-[-.07em]">
            Stay on our
            <br />
            frequency.
          </h2>
          <form className="self-end" onSubmit={(e) => e.preventDefault()}>
            <label htmlFor="email" className="text-[10px] font-semibold uppercase tracking-[.2em]">
              ATMOS transmissions
            </label>
            <div className="mt-5 flex border-b border-black pb-3">
              <input
                id="email"
                type="email"
                required
        ...
        <div className="mt-24 flex flex-col gap-5 border-t border-black/30 pt-5 text-[10px] font-semibold uppercase tracking-[.17em] md:mt-36 md:flex-row md:items-center">
          <Mark />
          ...
          <span className="md:ml-12">© ATMOS 2026</span>
        </div>
      </footer>
    </main>
  );
